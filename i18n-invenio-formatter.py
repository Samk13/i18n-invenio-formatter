...
def process_file(filepath):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.attr == 'format':
            ...
            if isinstance(func_call, ast.Call) and isinstance(func_call.func, ast.Name) and func_call.func.id in translation_aliases:
                # Get the original translation call
                start = calculate_offset(lines, func_call.lineno, func_call.col_offset)
                end = calculate_offset(lines, func_call.end_lineno, func_call.end_col_offset)
                ...
                format_kwargs = [f"{k.arg}={ast.unparse(k.value)}" for k in node.keywords]
                new_call = f'_("{modified_str}", {", ".join(format_kwargs)})'

                # Replace entire .format() call
                full_start = calculate_offset(lines, func_call.lineno, func_call.col_offset)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in translation_aliases:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Update InvenioRDM translation strings to use proper formatting')
